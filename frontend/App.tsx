...
import { 
  ...
  LogOut, Coffee, ArrowRight, ShieldAlert, Lock, Fingerprint, Delete, UserPlus, Save, ChevronLeft, ChevronRight, Calendar, History, Clock, Smartphone, X, Mic, MicOff, FileText, Cloud, ExternalLink, Briefcase, Phone, KeyRound, BellRing, Search, Download, CalendarDays, Zap, Wrench, Package, Info, Plus, Trash2, Timer, Filter, ChevronDown, Shield, AlertTriangle, AlertCircle, CheckCircle2, ScrollText
} from 'lucide-react';
...
import { Worker, Site, WorkLog, LogType, GeoLocationData, WorkMode, AdminUser, ToolRecord, AppConfig, WeeklyReport, Payroll } from './types';
import { AdminPanel } from './components/AdminPanel';
import { InstallTutorial } from './components/InstallTutorial';
import { ConfirmationModal } from './components/ConfirmationModal';
import { WeeklyReportModal } from './components/WeeklyReportModal';
import { WeeklyReportService } from './services/weeklyReportService';
import { WorkerPayrollsView } from './components/WorkerPayrollsView';
...
export const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      unsubWorkers(); unsubSites(); unsubLogs(); unsubAdmins(); unsubTools(); unsubConfig(); unsubWeekly(); unsubPayrolls();
    };
  }, []);
  ...
  const renderStep = () => {
    switch(currentStep) {
        ...
        <div className="flex flex-col h-full animate-fadeIn overflow-hidden pb-4">
           ...
           <div className="flex-1 bg-stone-900 border border-stone-800 rounded-[2.5rem] p-6 shadow-xl space-y-6 overflow-y-auto custom-scrollbar">
              <div className="space-y-3">
                 ...
                 <div className="flex gap-2">
                    {(['HORAS', 'DESTAJO'] as const).map(m => (
                      <button key={m} onClick={() => setExitWorkMode(m)} className={`flex-1 py-4 rounded-2xl text-xs font-black transition-all border ${exitWorkMode === m ? 'bg-amber-500 border-amber-500 text-white shadow-lg' : 'bg-stone-950 border-stone-800 text-stone-500'}`}>{m}</button>
                    ))}
                 </div>
              </div>
              <div className="space-y-3">
                 <label className="text-[10px] font-black text-stone-500 uppercase tracking-widest ml-1">Resumen de Tareas</label>
                 <textarea value={exitReportText} onChange={(e) => setExitReportText(e.target.value)} placeholder="¿Qué has hecho hoy?" className="w-full bg-stone-950 border border-stone-800 rounded-[2rem] p-5 text-sm text-white focus:border-amber-500 outline-none h-40 resize-none font-medium leading-relaxed" />
              </div>
              ...
              <button 
                ...
                className={`w-full py-5 rounded-[2rem] font-black uppercase text-xs tracking-widest flex items-center justify-center gap-3 transition-all shadow-2xl ${exitReportText.trim() ? 'bg-rose-600 text-white active:scale-95' : 'bg-stone-800 text-stone-500 cursor-not-allowed'}`}
              >
        ...
        return (
          <div className="flex flex-col h-full animate-fadeIn overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto space-y-3 pb-4 custom-scrollbar">
              ...
              {myReports.map(r => (
                <div key={r.id} data-testid={`worker-report-${r.id}`} className="bg-stone-900 rounded-2xl border border-stone-800 overflow-hidden">
                  <div className="flex gap-3 p-3">
                    {r.photoBase64 && <img src={r.photoBase64} alt="" className="w-20 h-20 object-cover rounded-xl shrink-0 border border-stone-800" />}
                    <div className="flex-1 min-w-0 flex flex-col justify-between">
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <p className="text-[10px] font-black text-amber-400 uppercase tracking-widest truncate">{r.weekStart} → {r.weekEnd}</p>
      ...
      case Step.WORKER_PAYROLLS:
        if (!selectedWorker) return null;
        return (
          <WorkerPayrollsView
            worker={selectedWorker}
            payrolls={payrolls}
            onBack={() => setCurrentStep(Step.WORKER_DASHBOARD)}
          />
        );
      case Step.WORKER_TOOLS: return (
        <div className="flex flex-col h-full animate-fadeIn overflow-hidden">
          ...
          <div className="relative mb-4 shrink-0"><Search className="absolute left-4 top-1/2 -transtone-y-1/2 text-stone-500" size={16}/><input type="text" placeholder="Buscar por nombre o marca..." className="w-full bg-stone-900 border border-stone-800 rounded-2xl py-4 pl-11 pr-4 text-xs text-white outline-none focus:border-amber-500" value={toolSearch} onChange={(e) => setToolSearch(e.target.value)}/></div>
          <div className="flex-1 overflow-y-auto space-y-3 pb-4 custom-scrollbar">
            ...
            <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-xl flex items-center justify-center p-6 animate-fadeIn">
              <div className="bg-stone-900 w-full max-w-sm rounded-[2.5rem] border border-stone-800 p-8 shadow-2xl relative">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-1.5"><label className="text-[9px] font-black text-stone-500 uppercase ml-1">Modelo</label><input type="text" className="w-full bg-stone-950 border border-stone-800 rounded-xl p-4 text-sm text-white outline-none" value={newToolForm.model} onChange={(e)=>setNewToolForm({...newToolForm, model: e.target.value})} /></div>
      ...
      case Step.REGISTER: return (
        <div className="flex flex-col h-full animate-fadeIn overflow-hidden pb-4">
           ...
           <div className="bg-stone-900 p-5 rounded-[2.5rem] border border-stone-800 space-y-3 shadow-xl overflow-y-auto custom-scrollbar flex-1">
             ...
             <input type="tel" placeholder="Teléfono" className="w-full bg-stone-950 border border-stone-800 rounded-xl p-4 text-sm text-white font-bold" value={regPhone} onChange={(e)=>setRegPhone(e.target.value)}/>
             <button onClick={handleRegistration} className="w-full bg-amber-500 text-white font-black py-4 rounded-2xl uppercase tracking-widest text-xs mt-4 active:scale-95 shadow-lg shrink-0">Registrarme</button>
           </div>
        </div>
      );
      default: return (
        <div className="flex items-center justify-center h-full text-stone-500 text-xs font-black uppercase tracking-[0.2em] animate-pulse">
           Cargando interfaz...
        </div>
      );
    }
  ...
  return (
    <div className="max-w-md mx-auto h-screen flex flex-col bg-stone-950 text-white p-4 relative overflow-hidden">
      {renderStep()}
      {showAdminLogin && (
        <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-xl flex items-center justify-center p-6 animate-fadeIn">
          <div className="bg-stone-900 w-full max-w-sm rounded-[2.5rem] border border-stone-800 p-8 shadow-2xl relative overflow-hidden">
             ...
             <div className="space-y-4">
                ...
                <button onClick={verifyAdminPassword} className="w-full bg-amber-500 py-4 rounded-xl font-black uppercase text-xs tracking-widest shadow-lg">Acceder al Panel</button>
             </div>
          </div>
        </div>
      )}
      {error && (<div className="fixed bottom-10 left-1/2 -transtone-x-1/2 bg-rose-600 px-6 py-3 rounded-full text-xs font-black uppercase z-[200] shadow-2xl flex items-center gap-3"><ShieldAlert size={16}/> {error} <button onClick={()=>setError('')} className="bg-white/20 p-1 rounded-full"><X size={12}/></button></div>)}
      <ConfirmationModal 
